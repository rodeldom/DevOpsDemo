pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                sh 'echo checkout'
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'GitHub-rodeldom', url: 'https://github.com/rodeldom/DevOpsDemo']])
            }
        }
        stage('Build') {
            steps {
            ...
            steps {
                script {
                    withCredentials([usernamePassword(
                        credentialsId: 'DockerHub-rodeldom',
                        ...
                        docker push rodeldom/devopsdemo:latest
                        '''
                    }
                }
            }
            ...
            steps {
                script {
                    withCredentials([string(
                        credentialsId: 'RenderDeployKey-DevOpsDemo',
                        variable: 'DEPLOY_URL'
                    )]) {
                        sh 'curl "$DEPLOY_URL"'
                    }
